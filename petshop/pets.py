...
def search(field, value):
    ...
    print(oby)
    if order == "asc":
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tag tg, tags_pets tp where tg.name = ? and tp.pet = p.id and tp.tag = tg.id and p.species = s.id",[value])
    else:
